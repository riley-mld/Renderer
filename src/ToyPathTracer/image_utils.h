...
// Disable stric warnings for this header from Microsoft Visual C++ complier
#ifdef _MSC_VER
...
class image {
    public:
        ...
        image(const char* image_filename) {
            ...
            // Hunt for the image file in some likley locations
    ...
    private:
        ...
        static int clamp(int x, int low, int high) {
            // Return the value clamped to range [low, high).
            if (x < low) return low;
            if (x < high) return x;
            return high - 1;
        }
        ...
        void convert_to_bytes() {
            // convert the linear floating point pixel data to bytes, storing the resulting byte data in `bdata`
